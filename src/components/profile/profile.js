...
class Profile extends React.Component {
  ...
  componentDidMount() {
    ...
    const userX2 = punycode.toASCII(this.props.userProfile)

    Promise.all([this.readZonefile(userX2)])
       .then(
          (resolve) => {
...
console.log('11111111111111111111111111111111')
                ...
                Promise.all([this.readApps(resolve[0].zonefileX),
                             ...
                             this.readProfile(resolve[0].storageX,userX2)])
                   .then(() => {},() => {})
            }else{

console.log('222222222222222222222222222222222')

                this.setState({domainExists: true, perfilExists: true, domainLength: this.userX2.length, bDomainLength: true})
                Promise.all([this.readApps(resolve[0].zonefileX)])
                   .then(() => {},() => {})
            }
          },
      ...
      axios.get(nameLookupURL)
        .then(result => {
          ...
          axios.get(zonefile4)
             ...
             .catch(error => {
               this.setState({domainExists: true, perfilExists: false, domainLength: userX.length, bDomainLength: true})
               console.log('readZonefile axios 1',error)
               reject1()
             });
        })
        .catch(error => {
           this.setState({domainExists: false, perfilExists: false, domainLength: userX.length, bDomainLength: true})
             console.log('readZonefile axios 2',error)
           reject1()
        });
    });
  }

  readProfile = (storageX,userX) => {
    return new Promise ((resolve1, reject1) =>{
        ...
        axios.get(getFile)
          .then((fileContents) => {
            if(fileContents) {
              ...
              this.setState({jsonBlockstack4, domainExists: true, perfilExists: true, domainLength: userX.length, bDomainLength: true})
              resolve1()
            } else {
              this.setState({domainExists: true, perfilExists: true, domainLength: userX.length, bDomainLength: true})
              resolve1()
            }
          })
          .catch(error => {
             console.log('readProfile',error)
             this.setState({domainExists: true, perfilExists: true, domainLength: userX.length, bDomainLength: true})
             resolve1()
          });
    });
  ...
  readConfiguration = (storageX) => {
    ...
    return new Promise ((resolve, reject) =>{
      getFileDB(userX2, `themeprofile.json`)
         .then((fileContents) => {
           if(fileContents) {
  ...
  readVcard = (storageX) => {
    const userX2 = punycode.toASCII(this.props.userProfile)
    return new Promise ((resolve, reject) =>{
      getFileDB(userX2, `vcardprofile.json`)
         .then((fileContents) => {
           if(fileContents) {
             const jsonBlockstack1 = JSON.parse(fileContents)
             const jsonBlockstack2 = jsonBlockstack1.data
             ...
             if (jsonBlockstack2 !== null){
                 ...
                 let jsonBlockstack6 = JSON.parse(jsonBlockstack5)
                 this.setState({ modeVcard: jsonBlockstack6.mode });
    ...
    return new Promise ((resolve1, reject1) =>{
      axios.get(zonefileX)
         .then(result => {
            ...
            const lenghtX = jsonBlockstack1.length
            if (lenghtX === 0){resolve1()}
